...
const HeaderInfoList = ({ product }) => {
  ...
  return (
    <div className='comparison_first_table_line_item_photos'>
      <div className='compared_prod_photo_body'>
        {' '}
        <img
          width='280'
          height='177'
          className='compared_prod_photo'
...
const ComparisonBody = () => {
  ...
  const [firstComparedProduct, secondComparedProduct] = useSelector(
    selectProductByCompared
  );
      ...
      <section className='comparison_body_section'>
        <div className='comparison_first_table_line'>
          <div className='comparison_first_table_line_item_header'>
            <h3 className='compared_request'>
              Go to Product page for more Products
            </h3>
            <Link className='compared_request_link' to='/shop'>
              View More
            </Link>
          </div>
          ...
          <div className='comparison_first_table_line_item_add_items'>
            <label className='nother_product_label' for='nother_prod'>
              Add A Product
            </label>
            <select
              className='nother_product_selector'
              onChange={(newProduct) => {
                dispatch(addProductToCompare(newProduct.target.value));
              }}
              name='nother_prod'
              id='nother_prod'
            >
          ...
          <table className='compare_table'>
              ...
              if (tableItem.header) {
                return (
                  <thead key={tableItem.key}>
                    <tr>
                      <th className='compare_table_header compare_table_text'>
                        {tableItem.text}
                      </th>
                ...
                return (
                  <tbody key={tableItem.key}>
                    <tr>
                      <td>
                        <p className='compare_text_table compare_item_first'>
                          {tableItem.text}
                        </p>
                      </td>
                      <td className='compare_table_item_pad compare_table_item compare_item_second'>
                        <p className='compare_text_table'>
                          {firstComparedProduct[tableItem.key]}
                        </p>
                      </td>
                      <td className='compare_table_item_pad compare_table_item compare_item_third'>
                        <p className='compare_text_table'>
                          {secondComparedProduct[tableItem.key]}
                        </p>
                      </td>
            ...
            <tbody>
              <tr>
                ...
                <td className='compare_item_btn'>
                  <button
                    onClick={() => {
                      addProductHandler(firstComparedProduct);
                    }}
                    type='button'
                    className='compare_table_btn'
                  >
                    Add To Cart
                  </button>
                </td>
                <td className='compare_item_btn'>
                  <button
                    onClick={() => {
                      addProductHandler(secondComparedProduct);
                    }}
                    type='button'
                    className='compare_table_btn'
                  >

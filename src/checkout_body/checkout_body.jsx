  ...
  return (
    <div key={checkoutInput.id} className='double_inputs_container'>
      <div className='double_inputs_item'>
        <label className='inputs_label' htmlFor={checkoutInput.id}>
          {checkoutInput.topText}
        </label>
        <input
          required
          className='input_text_global_pad double_checkout_input check_input'
          type={checkoutInput.type}
          id={checkoutInput.id}
        />
      </div>
      <div className='double_inputs_item'>
        <label className='inputs_label' htmlFor={checkoutInput.secondId}>
          {checkoutInput.additionalInformation.secondTopText}
        </label>
        <input
          required
          className='input_text_global_pad double_checkout_input check_input'
          type={checkoutInput.type}
          id={checkoutInput.additionalInformation.secondId}
        />
      </div>
    </div>
  );
...
const StandardInput = ({ checkoutInput }) => {
  return (
    <div key={checkoutInput.id} className='standard_check_input_container'>
      ...
      <input
        placeholder={checkoutInput.placeholder}
        required
        pattern={
        ...
        className='input_text_global_pad standard_check_input  check_input'
        type={checkoutInput.type}
        id={checkoutInput.id}
      />
    </div>
...
const ChekInput = ({ checkoutInput }) => {
  return (
    <div key={checkoutInput.id} className='standard_check_input_container'>
      ...
      <select
        required
        className='checkout_selector check_input'
        type={checkoutInput.type}
        name={checkoutInput.id}
        id={checkoutInput.id}
      >
        {checkoutInput.additionalInformation.countries.map((country) => {
          return (
            <option key={country.name} value={country.selectVal}>
              {country.name}
            </option>
          );
        })}
...
const checkoutInputsList = checkoutInputs.map((checkoutInput, index) => {
  if (checkoutInput.inputType === 'double') {
    return <DoubleInput checkoutInput={checkoutInput} />;
  } else if (checkoutInput.inputType === 'standard') {
    return <StandardInput checkoutInput={checkoutInput} />;
  } else {
    return <ChekInput checkoutInput={checkoutInput} />;
  }
});
      ...
      <div className='place_order'>
        <div className='checkout_product_container'>
          <div className='checkout_product_item'>
            <p className='checkout_product_header' data-testid='product'>
              Product
            </p>
          ...
          {selectedProducts.map((product) => {
            return (
              <div key={product.id} className='checkout_product_item'>
                <p className='checkout_product_name'>
                  {product.name}{' '}
                  <span className='checkout_product_quantity'>
                    X {product.quantity}
                  </span>
                </p>
                <p className='checkout_product_cost'>
        ...
        <form className='checkout_place_order' action=''>
          <div className='direct_input_block'>
            <input
              ...
              id='direct_bank'
            />
            <label className='direct_input_label' htmlFor='direct_bank'>
              Direct Bank Transfer
            </label>
          </div>

          <p className='make_sure_directly'>
          ...
          <div className='direct_input_block'>
            <input
              ...
              type='radio'
              name='bank'
              id='second_direct_bank'
            />
            <label className='direct_input_label' htmlFor='second_direct_bank'>
              Direct Bank Transfer
            </label>
          </div>
          <div className='direct_input_block cash_radio_block'>
            <input
              ...
              id='delivery'
            />
            <label className='direct_input_label' htmlFor='delivery'>
              Cash On Delivery
            </label>
          </div>

          <p className='personal_data_support'>
            Your personal data will be used to support your experience
            throughout this website, to manage access to your account, and for
            other purposes described in our{' '}
            <span className='bold_text'>privacy policy.</span>
          </p>
